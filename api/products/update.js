...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!productId || !name) {
      return sendJson(res, 400, { ok: false, error: 'Ürün ID ve ad zorunlu' });
    }

    const category = await ensureCategory(payload);

    const rows = await supabaseRequest(`/rest/v1/products?id=eq.${encodeURIComponent(productId)}`, {
      method: 'PATCH',
      headers: { Prefer: 'return=representation' },
      body: JSON.stringify({
        name,
    ...
    if (!Array.isArray(rows) || !rows.length) {
      return sendJson(res, 404, { ok: false, error: 'Ürün bulunamadı' });
    }
